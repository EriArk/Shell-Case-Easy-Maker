...
void main() {
  ...
  testWidgets('advanced sketch command creates semantic helper feature', (
    ...
    final addLine = find.byKey(const ValueKey('advanced-sketch-add-line'));
  ...
  testWidgets('selected sketch entity drags on workplane semantically', (
    tester,
  ) async {
    ...
    final fileService = _MemoryProjectFileService();
    final saveFile = File('drag_sketch_case.enclosure.json');
    final dialog = _FakeProjectFileDialogService(saveFile: saveFile);
    final sketch = advancedSketchWithEntities(
      const SemanticFeature(
        id: 'advanced_sketch_1',
    ...
    final dragEnd =
        canvasTopLeft +
        layout.workplaneLocalToCanvas(workplane, const Offset(24, 6));
    ...
    expect(movedEntity.parameters['center'], [24.0, 6.0]);
  ...
  testWidgets('selected circle sketch entity resizes from radius handle', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(1200, 900));
    addTearDown(() => tester.binding.setSurfaceSize(null));
    ...
    final fileService = _MemoryProjectFileService();
    final saveFile = File('radius_resize_circle_sketch_case.enclosure.json');
    final dialog = _FakeProjectFileDialogService(saveFile: saveFile);
    final sketch = advancedSketchWithEntities(
      const SemanticFeature(
        id: 'advanced_sketch_1',
        ...
        targetSurface: 'main_enclosure.front_wall.outer',
        operation: 'helper',
        parameters: {'name': 'Radius resize circle sketch'},
        metadata: {'advanced': true},
      ),
      [defaultSketchCircleEntity(id: 'circle_1')],
    );
    ...
    expect(
      find.byKey(
        const ValueKey(
          'advanced-sketch-circle-handle-advanced_sketch_1-circle_1-radius',
        ),
      ),
      findsNothing,
    );
    ...
    final dragStart = _frontWallCanvasPoint(tester, const Offset(6, 0));
    final dragEnd = _frontWallCanvasPoint(tester, const Offset(10, 0));
    final gesture = await tester.startGesture(
      dragStart,
      kind: PointerDeviceKind.mouse,
      buttons: kPrimaryMouseButton,
    ...
    expect(savedEntity.parameters['center'], [0.0, 0.0]);
    expect(savedEntity.parameters['diameter'], 20.0);
  });
  ...
  testWidgets(
    'selected rotated rectangle sketch entity resizes from edge handle',
    (tester) async {
      await tester.binding.setSurfaceSize(const Size(1200, 900));
      addTearDown(() => tester.binding.setSurfaceSize(null));

      final fileService = _MemoryProjectFileService();
      final saveFile = File('rotated_edge_resize_sketch_case.enclosure.json');
      final dialog = _FakeProjectFileDialogService(saveFile: saveFile);
      const rotation = 30.0;
      final sketch = advancedSketchWithEntities(
        const SemanticFeature(
          ...
          targetSurface: 'main_enclosure.front_wall.outer',
          operation: 'helper',
          parameters: {'name': 'Rotated edge resize sketch'},
          metadata: {'advanced': true},
        ),
        const [
          SketchEntity(
      ...
      final fixedEdge = _rotateTestOffset(const Offset(-10, 0), -rotation);
      final dragStartEdge = _rotateTestOffset(const Offset(10, 0), -rotation);
      final dragEndEdge = _rotateTestOffset(const Offset(16, 0), -rotation);
      final expectedCenter =
          fixedEdge + _rotateTestOffset(const Offset(13, 0), -rotation);
      final gesture = await tester.startGesture(
        _frontWallCanvasPoint(tester, dragStartEdge),
        kind: PointerDeviceKind.mouse,
        buttons: kPrimaryMouseButton,
      );
      await gesture.moveTo(_frontWallCanvasPoint(tester, dragEndEdge));
      await tester.pump();
      ...
      expect(savedEntity.parameters['width'], 26.0);
      expect(savedEntity.parameters['height'], 12.0);
      expect(savedEntity.parameters['rotation'], rotation);
    },
  );

  testWidgets(
    'selected rotated rectangle sketch entity edits corner radius handle',
    (tester) async {
      await tester.binding.setSurfaceSize(const Size(1200, 900));
      addTearDown(() => tester.binding.setSurfaceSize(null));
      ...
      final fileService = _MemoryProjectFileService();
      final saveFile = File(
        'rotated_corner_radius_handle_sketch_case.enclosure.json',
      );
      final dialog = _FakeProjectFileDialogService(saveFile: saveFile);
      const rotation = 30.0;
      final sketch = advancedSketchWithEntities(
        const SemanticFeature(
          ...
          targetSurface: 'main_enclosure.front_wall.outer',
          operation: 'helper',
          parameters: {'name': 'Rotated corner radius handle sketch'},
          metadata: {'advanced': true},
        ),
        const [
          SketchEntity(
      ...
      final radiusHandleFinder = find.byKey(
        const ValueKey(
          'advanced-sketch-rectangle-handle-advanced_sketch_1-rect_1-cornerRadius',
        ),
      );
      expect(radiusHandleFinder, findsOneWidget);
      expect(tester.getSize(radiusHandleFinder), const Size.square(16));

      final dragStart = _rotateTestOffset(const Offset(8, 4), -rotation);
      final dragEnd = _rotateTestOffset(const Offset(5, 1), -rotation);
      final gesture = await tester.startGesture(
        _frontWallCanvasPoint(tester, dragStart),
        kind: PointerDeviceKind.mouse,
        buttons: kPrimaryMouseButton,
      );
      await gesture.moveTo(_frontWallCanvasPoint(tester, dragEnd));
      await tester.pump();
      ...
      final savedEntity = sketchEntitiesForFeature(savedSketch).single;
      expect(savedEntity.parameters['center'], [0.0, 0.0]);
      expect(savedEntity.parameters['width'], 20.0);
      expect(savedEntity.parameters['height'], 12.0);
      expect(savedEntity.parameters['cornerRadius'], 5.0);
      expect(savedEntity.parameters['rotation'], rotation);
    },
  );
  ...
  testWidgets('circle placement drag draws semantic sketch circle', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(1200, 900));
    addTearDown(() => tester.binding.setSurfaceSize(null));
    ...
    final fileService = _MemoryProjectFileService();
    final saveFile = File('drag_draw_circle_sketch_case.enclosure.json');
    final dialog = _FakeProjectFileDialogService(saveFile: saveFile);
    final sketch = advancedSketchWithEntities(
      const SemanticFeature(
        id: 'advanced_sketch_1',
        ...
        targetSurface: 'main_enclosure.front_wall.outer',
        operation: 'helper',
        parameters: {'name': 'Drag draw circle sketch'},
        metadata: {'advanced': true},
      ),
      const [],
    );
    ...
    final dragStart = _frontWallCanvasPoint(tester, const Offset(10, -2));
    final dragEnd = _frontWallCanvasPoint(tester, const Offset(16, 6));
    final gesture = await tester.startGesture(
      dragStart,
      kind: PointerDeviceKind.mouse,
      buttons: kPrimaryMouseButton,
    ...
    expect(
      find.byKey(
        const ValueKey(
          'advanced-sketch-entity-draw-preview-advanced_sketch_1-circle',
        ),
      ),
      findsOneWidget,
    );
    ...
    final savedEntity = sketchEntitiesForFeature(savedSketch).single;
    expect(savedEntity.id, 'circle_1');
    expect(savedEntity.parameters['center'], [10.0, -2.0]);
    expect(savedEntity.parameters['diameter'], 20.0);
  });

  testWidgets('line placement drag draws semantic sketch line', (tester) async {
    await tester.binding.setSurfaceSize(const Size(1200, 900));
    addTearDown(() => tester.binding.setSurfaceSize(null));

    final fileService = _MemoryProjectFileService();
    final saveFile = File('drag_draw_line_sketch_case.enclosure.json');
    final dialog = _FakeProjectFileDialogService(saveFile: saveFile);
    final sketch = advancedSketchWithEntities(
      const SemanticFeature(
        id: 'advanced_sketch_1',
        ...
        targetSurface: 'main_enclosure.front_wall.outer',
        operation: 'helper',
        parameters: {'name': 'Drag draw line sketch'},
        metadata: {'advanced': true},
      ),
      const [],
    );
    ...
    await tester.tap(sketchRow.first);
    await _pumpAsyncUi(tester);
    await tester.tap(find.byKey(const ValueKey('advanced-sketch-add-line')));
    await _pumpAsyncUi(tester);

    expect(
      find.byKey(
        const ValueKey('advanced-sketch-entity-line-placement-active'),
      ),
      findsOneWidget,
    );

    final dragStart = _frontWallCanvasPoint(tester, const Offset(-8, -3));
    final dragEnd = _frontWallCanvasPoint(tester, const Offset(18, 7));
    final gesture = await tester.startGesture(
      dragStart,
      kind: PointerDeviceKind.mouse,
      buttons: kPrimaryMouseButton,
    ...
    expect(
      find.byKey(
        const ValueKey(
          'advanced-sketch-entity-draw-preview-advanced_sketch_1-line',
        ),
      ),
      findsOneWidget,
    );
    ...
    final savedEntity = sketchEntitiesForFeature(savedSketch).single;
    expect(savedEntity.id, 'line_1');
    expect(savedEntity.type, 'line');
    expect(savedEntity.parameters['start'], [-8.0, -3.0]);
    expect(savedEntity.parameters['end'], [18.0, 7.0]);
  });

  testWidgets('unselected line sketch entity drags directly from viewport', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(1200, 900));
    addTearDown(() => tester.binding.setSurfaceSize(null));
    ...
    final fileService = _MemoryProjectFileService();
    final saveFile = File('direct_drag_line_sketch_case.enclosure.json');
    final dialog = _FakeProjectFileDialogService(saveFile: saveFile);
    final sketch = advancedSketchWithEntities(
      const SemanticFeature(
        id: 'advanced_sketch_1',
        ...
        targetSurface: 'main_enclosure.front_wall.outer',
        operation: 'helper',
        parameters: {'name': 'Direct drag line sketch'},
        metadata: {'advanced': true},
      ),
    ...
    final dragStart = _frontWallCanvasPoint(tester, Offset.zero);
    final dragEnd = _frontWallCanvasPoint(tester, const Offset(6, -5));
    final gesture = await tester.startGesture(
      dragStart,
      kind: PointerDeviceKind.mouse,
      buttons: kPrimaryMouseButton,
    ...
    final savedEntity = sketchEntitiesForFeature(savedSketch).single;
    expect(savedEntity.parameters['start'], [-4.0, -5.0]);
