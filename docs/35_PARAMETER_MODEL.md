...
side. Inspector nudge and resize actions reuse these normalized values so each
movement or size edit stays a semantic undoable edit. When a supported sketch
workplane size is available, the adapter can also report a warning if the
rectangle extends outside that workplane. These values remain semantic helper
